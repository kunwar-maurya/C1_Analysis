"""Bronze: ingest products.csv into Delta."""
...
def ingest_products(config=None, overrides=None, spark=None):
    config = apply_runtime_overrides(config or load_config(), overrides)
    spark = spark or get_spark("bronze-ingest-products")
    ...
    source = raw_file_path(config, "products.csv")
    df = read_csv_with_schema(spark, source, PRODUCTS_SCHEMA)
    run_id = (overrides or {}).get("pipeline_run_id")
    df = with_ingestion_metadata(df, source_file=source, pipeline_run_id=run_id)

    table = fully_qualified_table(config, "bronze", "products")
    write_delta(df, table)

    count = df.count()
    print(f"Bronze products written to {table}; rows={count}")
